export const towerBuilder = (nFloors: number): string[] => {
  ...
  for (let i = 0; i < nFloors; i++) {
    ...
    const sideSpaceCount = calculateSideSpace(asteriskCount, finalAsteriskCount);
...
function calculateSideSpace(amountOfAsterisks: number, bottomLayerAsterisksCount: number): number {
  return (bottomLayerAsterisksCount - amountOfAsterisks) / 2;
}

function insertSpace(asterisks: string, spaceAmount: number, spaceSymbol: string): string {
